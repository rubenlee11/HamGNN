...
class KANReadoutBlock(torch.nn.Module):
    def __init__(
        ...
        dim = [MLP_irreps.dim, MLP_irreps.dim // 2, MLP_irreps.dim // 4, 1]
        self.kan = MultKAN(width=dim, grid=3, k=3, mult_arity=2, symbolic_enabled=False, auto_save=False,
                           save_act=False)
        # self.kan.speed(compile=True)

    def forward(self, x: torch.Tensor) -> torch.Tensor:  # [n_nodes, irreps]  # [..., ]
        x = self.linear(x)
        return self.kan(x)  # [n_nodes, 1]
...
class RealAgnosticResidualInteractionBlock(InteractionBlock):
    def _setup(self) -> None:
        ...
        input_dim = self.edge_feats_irreps.num_irreps
        self.conv_tp_weights = nn.FullyConnectedNet(
            [input_dim] + [input_dim * 2, (input_dim * 2 + self.conv_tp.weight_numel // 2) // 2,
                           self.conv_tp.weight_numel // 2] + [self.conv_tp.weight_numel],
            torch.nn.functional.silu,
        )
...
@compile_mode("script")
class ParaRealAgnosticResidualInteractionBlock2(InteractionBlock):
    def _setup(self) -> None:
        # First linear
        self.linear_up = o3.Linear(
            self.node_feats_irreps,
    ...
    def forward(
        ...
        tp_weights = self.conv_tp_weights(edge_feats)
        mji = self.conv_tp(
            node_feats[receiver], edge_attrs, tp_weights
        )  # [n_edges, irreps]
        message = scatter_sum(
            src=mji, index=receiver, dim=0, dim_size=num_nodes
        )  # [n_nodes, irreps]
...
@compile_mode("script")
class ParaRealAgnosticInteractionBlock2(InteractionBlock):
    def _setup(self) -> None:
        # First linear
        self.linear_up = o3.Linear(
            self.node_feats_irreps,
...
@compile_mode("script")
class ComplexRealAgnosticResidualInteractionBlock(InteractionBlock):
    def _setup(self) -> None:
        # First linear
        self.linear_up = o3.Linear(
            self.node_feats_irreps,
    ...
    def forward(
        ...
        tp_weights = self.conv_tp_weights(edge_feats)
        mji = self.conv_tp(
            node_feats[sender], node_feats[receiver], tp_weights
        )  # [n_edges, irreps]
        message = scatter_sum(
            src=mji, index=receiver, dim=0, dim_size=num_nodes
        )  # [n_nodes, irreps]
...
@compile_mode("script")
class ParaRealAgnosticResidualInteractionBlock(InteractionBlock):
    def _setup(self) -> None:
        self.edge_linear = o3.FullyConnectedTensorProduct(
            self.edge_attrs_irreps, self.edge_feats_irreps, self.edge_attrs_irreps
        )
    ...
    def forward(
        ...
        sc = self.skip_tp(node_feats, node_attrs)
        # node_feats = self.linear_up(node_feats)
        edge_attrs = self.edge_linear(edge_attrs, edge_feats)
        message = scatter_sum(
            src=edge_attrs, index=receiver, dim=0, dim_size=num_nodes
        )  # [n_nodes, irreps]
        tp_weights = self.conv_tp_weights(node_attrs)
        message = self.conv_tp(
            node_feats, message, tp_weights
        )  # [n_nodes, irreps]
        message = self.linear(message) / self.avg_num_neighbors
...
@compile_mode("script")
class ParaRealAgnosticInteractionBlock(InteractionBlock):
    def _setup(self) -> None:
        self.edge_linear = o3.FullyConnectedTensorProduct(
            self.edge_attrs_irreps, self.edge_feats_irreps, self.edge_attrs_irreps
        )
